...
export class ArticleService {
  ...
  constructor( private http: Http ) {
    this.orderedArticles =
      Observable.combineLatest(this._articles, this._sortByFilterSubject, this._sortByDirectionSubject)
      .map(([
...
public getArticles() : void{
  ...
  this._makeHttpRequest('/v1/articles', 'cnn')
  // convert response into artivle class
...
): Observable<any>
{
  ...
  return this.http
          ...
          //.map(json => json.articles)
          //.map(articles => articles.map(article=> Article.fromJSON(article)));
}
